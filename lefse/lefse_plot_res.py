...
import pandas as pd
colors = sns.color_palette('Set2')+sns.color_palette('Set3')+sns.color_palette('Pastel2')+sns.color_palette('Pastel1')
mapping = pd.read_csv("/home/qiime2/input/metadata/mapping.txt",sep='\t',index_col=0)
groups = len(set(mapping[mapping.columns[-1]].to_list()))
...
def read_params(args):
    ...
    parser.add_argument('--feature_font_size', dest="feature_font_size", type=int, default=8, help="the file for the output image")
    ...
    parser.add_argument('--class_legend_font_size',dest="class_legend_font_size", type=str, default="8")
    parser.add_argument('--width',dest="width", type=float, default=10.0 )
    parser.add_argument('--height',dest="height", type=float, default=6.0, help="only for vertical histograms")
...
def plot_histo_hor(path,params,data,bcl,report_features):
    ...
    for i,v in enumerate(data['rows']):
        ...
        ax.barh(pos[i],vv, align='center', color=col, label=lab, height=0.8, edgecolor='#595959', linewidth=0.8)
    ...
    ax.set_yticks([])
    ax.set_xlabel("LDA SCORE (log 10)",fontsize=params['feature_font_size']+1)
    ax.tick_params(axis='x', labelsize= params['feature_font_size'])
    ...
    leg = ax.legend(ncol=1, bbox_to_anchor=(1,1),loc='upper left', prop={'size':params['class_legend_font_size']})
    #leg = ax.legend(ncol=5, borderaxespad=0., frameon=False, prop={'size':params['class_legend_font_size']})
    ...
    #leg = ax.legend(ncol=4, frameon=False,prop={'size':params['class_legend_font_size']})
    ...
    plt.tight_layout()
    plt.savefig(path,format=params['format'],facecolor=params['back_color'],edgecolor=params['fore_color'],dpi=params['dpi'],bbox_inches='tight')
    plt.close()
...
def plot_histo_ver(path,params,data,report_features):
    ...
    for i,v in enumerate(data['rows']):
        ...
        col = colors[indcl%len(colors)]
        vv = fabs(float(v[3])) 
        ax.bar(pos[i],vv, align='center', color=col, label=lab, linewidth = 0)
    if report_features:
        print('OTU\tLDA_score\tCLass')
        for i in out_data:
            print('%s\t%s\t%s' %(i, out_data[i][0], out_data[i][1]))
