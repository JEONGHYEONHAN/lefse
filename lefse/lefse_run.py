...
def lefse_run():
    ...
    save_res(outres,params["output_file"]+"_total.res") # _filtered.res : original result file, 유의하지 않은(LDA<2 또는 p-value >0.05) taxonomy에 대한 정보는 빈값
    
    df = pd.read_csv(params["output_file"]+"_total.res", sep = '\t', header=None, index_col=None)
    df.columns = ['taxonomy','log mean','class','lda','pvalue']
    
    df['lda'] = df['lda'].astype(float)
    ...
    print(outres.keys())
    save_res(outres,params["output_file"]+"_total.res") # _total.res : total result file, 유의한 taxonomy에 대한 정보는 빈값
